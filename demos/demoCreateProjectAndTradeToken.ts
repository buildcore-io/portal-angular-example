...
const token = await build5.dataset(Dataset.TOKEN).create(await signWithMetamask({
    access: 0,
    name: 'Test token',
    symbol: Math.random().toString(36).substr(2, 6).replace(/[0-9]/g, '').toUpperCase(),
    decimals: 6,
...
build5.dataset(Dataset.TOKEN).id(token.uid).getLive().subscribe((t) => { console.log('Token record updated - ', t.uid,', current status: ', t.status ,', full updates: ', t) });
...
build5.dataset(Dataset.TOKEN_MARKET).getBidsLive(token.uid, TokenTradeOrderType.SELL, TokenTradeOrderStatus.ACTIVE).subscribe((t) => { console.log('Current sell orders on the book: ', t) });
